...
//   // Run user code and check output
//   const handleSubmit = async () => {
//     if (!selectedProblem || !user) return;
//     setRunning(true);

//     try {
//       // Wrap user code inside a function
//       const func = new Function("input", code);

//       const parsedInput = JSON.parse(selectedProblem.input); // convert string -> array/object
//       const got = func(parsedInput); // run user code
//       const expected = JSON.parse(selectedProblem.output);
...
//           };
//           setUser(updatedUser);
//           await AsyncStorage.setItem("user", JSON.stringify(updatedUser)); // 🔥 save updated user
//         }
//       } else {
//         Alert.alert(
//           "❌ Wrong",
...
//   const problems =
//     (problemsData as Record<string, Problem[]>)[topic as string] || [];
...
//               {prob.statement.slice(0, 60)}...
//             </Text>
//             {user?.solvedProblems?.includes(prob.id) && (
//               <Text style={{ color: "green", marginTop: 4 }}>✅ Solved</Text>
//             )}
//           </TouchableOpacity>
//         ))}
//       </ScrollView>
...
export default function TopicScreen() {
  ...
  function safeParse(value: any) {
    ...
    try {
      ...
      if (/^\[.*\]$/.test(val)) {
        try {
          return eval(val); // fallback
        } catch {
          return val;
        }
      }
      // Handle numbers in string "1 2 3 4"
      if (/^-?\d+(\s+-?\d+)*$/.test(val)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 👇 always safe-parse both
      const parsedInput = safeParse(selectedProblem.input);
      const expected = safeParse(selectedProblem.output);

      // run user code
      const got = func(parsedInput);
